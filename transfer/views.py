    ...
    if request.method == 'POST': 
        ...
        if form.is_valid(): 
            ...
            if move_exit:
                ...
                generic.addEvent(request, 'Local move occurred from "%s" to "%s" in the amount of %s %s' % (from_account['name'], to_account['name'], amount, currency.upper()), 'info')
                ...
                generic.addEvent(request, 'Transfer initialized from "%s" to "%s" of %s %s' % (from_account['name'], to_address, amount, currency.upper()), 'info')
